...
│   ├── internal/             # Internal documentation (this file)
...
### Documentation Deployment (`gh-pages.yml`)
- **Triggers:** Version tags (`v*.*.*`), releases, manual dispatch
- **Process:** 
  1. Installs released package from PyPI
  2. Builds documentation with `mkdocs build --clean`
  3. Deploys to GitHub Pages

### Package Publishing (`publish.yml`)
- **Triggers:** Version tags (`v*.*.*`)
- **Process:**
  1. Builds package with `uv build`
  2. Publishes to PyPI

## 📝 Notes

- The documentation automatically pulls the version number from `pyproject.toml` via the macros plugin
- The project uses `uv` for dependency management but also supports `pip`
- All development dependencies are specified in the `[dependency-groups.dev]` section of `pyproject.toml`
- The release script handles both semantic versioning and git operations automatically
- Documentation changes should be tested locally with `mkdocs serve` before committing

The project has a well-structured build and release process that automatically handles documentation deployment and package publishing through GitHub Actions!