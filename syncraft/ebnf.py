...
from syncraft.syntax import Syntax, SyntaxSpec, Graph, SeqSpec, AltSpec, ManySpec, LexSpec, LazySpec, EntryCategory
...
class EBNF(Grammar):
    ...
    erule = S.rp(
        ...
        ident=ident,
        expr=expr,
    ).to(lambda env: (env.ident, env.expr), lambda env: RuleDef(env.ident, env.expr)).format("{0} = {1};\n")
