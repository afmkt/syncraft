...
# --- DFA.to_dict/from_dict tests ---
def test_dfa_to_dict_and_from_dict_ascii():
    from syncraft.fa import NFA, DFA
    u = CodeUniverse.ascii()
    nfa = NFA.from_charset('a', universe=u).then(NFA.from_charset('b', universe=u))
    dfa = nfa.dfa
    d = dfa.to_dict()
    dfa2 = DFA.from_dict(d)
    # DFA structure should be preserved
    assert dfa2.universe.value == dfa.universe.value
    assert dfa2.universe.space is str
    assert dfa2.init.id == dfa.init.id
    assert set(dfa2.accept.keys()) == set(dfa.accept.keys())
    # Acceptance should match
    assert match(dfa, 'ab') == match(dfa2, 'ab')
    assert match(dfa, 'a') == match(dfa2, 'a')

def test_dfa_to_dict_and_from_dict_set():
    from syncraft.fa import DFA, FAState
    u = CodeUniverse.set(frozenset({'A', 'B'}))
    s0 = FAState(0)
    s1 = FAState(1)
    transitions = {s0: {}, s1: {}}
    accept = {s1: frozenset()}
    ...
    dfa2 = DFA.from_dict(d)
    assert dfa2.universe.value == dfa.universe.value
    assert dfa2.universe.space == dfa.universe.space
    assert dfa2.init.id == dfa.init.id
    assert set(dfa2.accept.keys()) == set(dfa.accept.keys())

def test_dfa_to_dict_and_from_dict_enum():
    from syncraft.fa import DFA, FAState
    import enum
    ...
    assert dfa2.universe.value == dfa.universe.value
    assert dfa2.universe.space is Fruit
    assert dfa2.init.id == dfa.init.id
    assert set(dfa2.accept.keys()) == set(dfa.accept.keys())

if __name__ == "__main__":
    test_then()  # smoke test