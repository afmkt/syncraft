...
def test_parse_text_input_without_config_infers_lexer() -> None:
    ...
    value, state = parser_run(syntax=syntax, data=Input.from_data(["hi"]))
...
def test_parse_bytes_input_without_config_infers_lexer() -> None:
    ...
    value, state = parser_run(syntax=syntax, data=Input.from_data([b"\x01"]))
...
def test_parse_string_input_with_lexer_bind() -> None:
    ...
    value, state = parser_run(syntax=word, data=Input.from_data("hi"))
...
def test_parse_bytes_input_with_lexer_bind() -> None:
    ...
    value, state = parser_run(syntax=byte_token, data=Input.from_data(b"\x01"))
