...
from syncraft.syntax import AltSpec, LexSpec
...
def test_unary_seq_default_mode_keeps_singleton_tuple() -> None:
    ...
    assert value == "x", f"Expected 'x', got {value!r}"
...
def test_alt_default_mode_keeps_nested_choice_shape() -> None:
    ...
    assert isinstance(left.spec.options[0], LexSpec)
    assert isinstance(right.spec.options[1], LexSpec)
