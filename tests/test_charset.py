...
def test_charset_basic_matches() -> None:
    cc: CharClass[str] = CharClass.charset("abc", universe=CodeUniverse.ASCII)
    assert cc("a")
    assert cc("b")
    assert cc("c")
    assert not cc("d")
...
def test_charset_union_and_interval_merge() -> None:
    a: CharClass[str] = CharClass.charset("A", universe=CodeUniverse.ASCII)
    b: CharClass[str] = CharClass.charset("B", universe=CodeUniverse.ASCII)
...
def test_charset_universe_mismatch() -> None:
    ascii_a: CharClass[str] = CharClass.charset("A", universe=CodeUniverse.ASCII)
    uni_a: CharClass[str] = CharClass.charset("A", universe=CodeUniverse.UNICODE)
...
def test_charset_invalid_length_error() -> None:
    cc: CharClass[str] = CharClass.charset("A", universe=CodeUniverse.ASCII)
    with pytest.raises(CodepointError):
        cc("AB")  # multi-character should raise
    cc_bytes: CharClass[bytes] = CharClass.charset(b"A", universe=CodeUniverse.BYTE)
    with pytest.raises(CodepointError):
        cc_bytes(b"AB")
